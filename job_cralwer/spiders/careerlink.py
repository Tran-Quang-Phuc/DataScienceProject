...
class CareerlinkSpider(scrapy.Spider):
    ...
    def parse_link(self, response):   
        for i in range(1,51):
            str ='body > div.container.mt-3.mt-lg-4 > div > div.col-lg-9.px-0.px-lg-3 > ul > li:nth-child({}) > div > div.media-body.overflow-hidden > a.job-link.clickable-outside::attr(href)'.format(i)
            updateDateStr='body > div.container.mt-3.mt-lg-4 > div > div.col-lg-9.px-0.px-lg-3 > ul > li:nth-child({}) > div > div.media-body.overflow-hidden > div.d-lg-flex.justify-content-between > div.align-items-center.justify-content-between > span > span::text'.format(i)
            ...
            yield scrapy.Request(url=link, callback=self.parse_job,meta={'link': link,'update':updateDate, 'deadline':deadlineDate})
    ...
    def parse_job(self, response):
        ...
        job_item['job_title'] = response.css('#jd-col > div > div.card.border-0.font-nunitosans.px-4 > div.job-detail-header.mt-3 > div.media.row.m-0 > div.media-body.job-title-and-org-name > div > h1::text').get(default='').strip()
        ...
        raw_listed= response.css('#jd-col div.job-detail-header.mt-3 div.job-overview.mt-2 div.date-from.d-flex.align-items-center span').get()
        job_item['job_listed'] = re.search(r'\b(\d{1,2}-\d{1,2}-\d{4})\b', raw_listed).group(1)
       
        job_item['salary'] = response.css('#jd-col div div.card.border-0.font-nunitosans.px-4 div.job-detail-header.mt-3 div.job-overview.mt-2 span.text-primary::text').get(default='not-found').strip()

        job_item['job_deadline'] = response.css('#jd-col div.card.border-0.font-nunitosans.px-4 div.job-detail-header.mt-3 div.job-overview.mt-2 div.day-expired.d-flex.align-items-center span b::text').get(default='not-found').strip()
        job_item['job_deadline'] = job_item['job_deadline'].replace('\n', ' ')


        job_item['company_name'] = response.css('#jd-col > div > div.card.border-0.font-nunitosans.px-4 > div.job-detail-header.mt-3 > div.media.row.m-0 > div.media-body.job-title-and-org-name > p > a > span::text').get(default='not-found').strip()
        ...
        industriesText = response.css('#section-job-summary div.row.job-summary.d-flex div.col-6.pl-1.pr-3.pl-md-2 div a span::text').getall()
        ...
        job_item['job_description']=', '.join([description.strip() for description in descriptionText if description.strip()])
