...
def get_min_max_salary(salary_string:str):
    ...
    elif "Tới" in salary_string:
        max_salary = float(salary_string.split(" ")[1])
        unit = salary_string.split(" ")[-1]
        if unit.lower() == "triệu":
            max_salary = int(min_salary * 1000000)
        elif unit.lower() == "usd":
            max_salary = int(min_salary * 24000)
        return {"min": None, "max": max_salary}
